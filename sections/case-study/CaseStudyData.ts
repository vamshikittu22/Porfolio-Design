...
└── Prod_Release (Vercel Integration)
      ...
      code: {
        title: "Vitest Service Mocking",
        lang: "typescript",
        code: `// geminiService.test.ts
import { describe, it, expect, vi } from 'vitest';

describe('GeminiService', () => {
  it('should trigger quota lock on 429', async () => {
    const service = GeminiService.getInstance();
    // Force a mock quota error
    vi.spyOn(global, 'fetch').mockResolvedValue({ 
      status: 429, 
      json: () => Promise.resolve({ error: 'Limit reached' }) 
    });
    
    await expect(service.generateImage('test')).rejects.toThrow('QUOTA_EXCEEDED');
...
└── Auth_Layer (Scoped API Tokens)
      ...
      code: {
        ...
        lang: "typescript",
        code: `// Secure Header Policy
const CSP_CONFIG = {
  'default-src': "'self'",
  'script-src': ["'self'", "'unsafe-eval'", "esm.sh"],
  'connect-src': ["'self'", "*.googleapis.com", "*.github.com"],
    ...
    rollupOptions: {
      output: {
        manualChunks: {
          ...
          'game-engine': ['./sections/game/TicTacToe.tsx']
        }
      }
    }
  }
