...
import { RESUME_CONTENT, Certification } from '../data/ResumeData';
...
export const ResumeCertifications: React.FC = () => {
    ...
    const renderCert = (cert: Certification, idx: number) => {
        const content = (
            <div
                className="group relative flex flex-col items-center p-4 rounded-2xl bg-t-fg/[0.03] dark:bg-white/[0.03] border border-t-border/10 hover:border-t-accent/30 transition-all duration-300 hover:scale-[1.05] print:border-0 print:bg-transparent print:p-2 cursor-pointer h-full"
            >
    ...
    return (
        <section className="break-avoid">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 print:grid-cols-3 print:gap-4">
                {RESUME_CONTENT.certifications.map((cert, idx) => renderCert(cert, idx))}
            </div>
        </section>
    );
};
