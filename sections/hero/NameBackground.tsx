...
// Technical vocabulary used for the procedural generation
const VOCABULARY = [
...
// Theme-aware ink colors defined in index.html :root
const COLORS = [
    "var(--color-accent)",
...
export const NameBackground: React.FC = () => {
    const items = useMemo(() => {
        ...
        const count = 550; // Total nodes in the constellation
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Tiered density logic for architectural depth (Macro to Micro)
            if (roll < 0.05) {
                sizeValue = Math.random() * 6 + 5; // Macro Layer (Very Large)
                opacityBase = 0.08;
                blurValue = "blur(3px)";
            } else if (roll < 0.2) {
                ...
                sizeValue = Math.random() * 0.4 + 0.3; // Texture Layer (Tiny)
                opacityBase = 0.35;
                blurValue = "blur(0px)";
            }

            const rotations = [0, 0, 0, 90, -90, 0]; // Swiss alignment constraints
    ...
    return (
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden select-none">
            ...
            {/* Editorial Vignette Layers (Prevents edges from being too sharp) */}
