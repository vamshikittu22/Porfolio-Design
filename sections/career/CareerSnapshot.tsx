  ...
  const combinedData = useMemo(() => {
    const combined: CareerItem[] = [
      ...EXPERIENCE.map((item, idx) => {
        ...
        const yearStr = years && years.length > 0 ? years[0] : '2025';
        return { 
          ...item, 
          id: `work-${idx}`,
          type: 'work' as const, 
      ...
      ...EDUCATION.map((item, idx) => {
        const years = item.period?.match(/\d{4}/g);
        const yearStr = years && years.length > 0 ? years[0] : '2024';
        return { 
          ...item, 
          id: `edu-${idx}`,
          ...
          <div className="flex flex-col gap-8 mb-12">
            <div>
              <h2 className="text-4xl lg:text-6xl font-black font-display text-t-fg uppercase tracking-tighter leading-none mb-3">
                Career <br /> <span className="text-t-accent">Timeline.</span>
              </h2>
              <p className="text-[9px] font-black uppercase tracking-[0.5em] text-t-fg-m opacity-50">
                Professional Journey & Academic Milestone
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                { val: "3", label: "Enterprises", accent: "orange" },
                { val: "2", label: "Degrees", accent: "indigo" },
                { val: "10+", label: "Tech Stack", accent: "emerald" }
              ].map((stat, i) => (
                <div key={i} className="flex flex-col items-center justify-center text-center p-4 rounded-3xl bg-t-bg/40 border border-t-border/20 transition-transform hover:-translate-y-1">
                    ...
                    >
                      <div 
                        className={`
                          ...
                          ${isActive ? 'shadow-2xl border-t-accent/40' : ''}
                        `}
                      >
                          ...
                          <ul className="space-y-3">
                            {item.description.slice(0, 3).map((bullet, bIdx) => (
                              <li key={bIdx} className="flex gap-3 items-start">
                                <div className="w-1.5 h-1.5 rounded-full bg-t-accent mt-1.5 flex-shrink-0" />
